defmodule Importer.Distance do
  ...
  def track_index(dist, item, track_info) do
    track_indexes =
      [track_info.medium_index, track_info.index]
      |> Enum.map(&to_string/1)

    add_expr(dist, :track_index, !(to_string(item.index) not in track_indexes))
  end
  ...
  def album_distance(items, release, mapping, likelies) do
    ...
    |> add_if(
      likelies["DISCTOTAL"] && release.mediums,
      fn dist -> add_number(dist, :disctotal, likelies["DISCTOTAL"], release.mediums) end
    )
    |> add_if(
      likelies["MUSICBRAINZ_ALBUMID"] && release.album_id,
      fn dist ->
        add_expr(dist, :album_id, likelies["MUSICBRAINZ_ALBUMID"] == release.album_id)
      end
    )
    |> add_if(
      likelies["ALBUMDISAMBIG"] && release.albumdisambig,
      fn dist ->
        add_string(dist, :albumdisambig, likelies["ALBUMDISAMBIG"], release.albumdisambig)
      end
    )
    |> add_if(
      likelies["CATALOGNUMBER"] && release.catalognum,
      fn dist -> add_string(dist, :catalognum, likelies["CATALOGNUMBER"], release.catalognum) end
    )
    |> add_if(
      likelies["LABEL"] && release.label,
      fn dist -> add_string(dist, :label, likelies["LABEL"], release.label) end
    )
    |> add_if(
      likelies["COUNTRY"] && release.country,
