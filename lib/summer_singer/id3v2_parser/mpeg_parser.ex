defmodule MPEGParser do
  ...
  defp mpeg_version(mpeg_version) do
    case mpeg_version do
      0 -> "MPEG Version 2.5"
      1 -> "Reserved"
      2 -> "MPEG Version 2"
      3 -> "MPEG Version 1"
  ...
  defp layer_description(layer_description) do
    case layer_description do
      0 -> "Reserved"
      1 -> "Layer III"
      2 -> "Layer II"
      3 -> "Layer I"
  ...
  defp bitrate_index(bitrate_index) do
    case bitrate_index do
  ...
  defp sampling_index(sampling_index) do
    case sampling_index do
      0 -> [44100, 22050, 11025]
      1 -> [48000, 24000, 12000]
      2 -> [32000, 16000, 8000]
  ...
  defp channel_mode(channel_mode) do
    case channel_mode do
      0 -> "Stereo"
  ...
  defp get_duration(bitrate, samplerate, file_size, audio_data, mpeg_version) when
    ...
    cond do
      ...
      # Should be 32 bytes after the end of the first MPEG audio'header
      # :nomatch != :binary.match(audio_data, [<< "VBRI" >>]) ->
      #  vbri_match = :binary.match(audio_data, [<< "VBRI" >>])
      #  {tag_position, 4} = vbri_match
      #  vbri_header = :binary.part(audio_data, tag_position, 26)

      #  << "VBRI",
      #    _version :: bytes-size(2),
      #      _delay :: bytes-size(2),
      #    _quality :: bytes-size(2),
      #       bytes :: integer-size(32),
      #      frames :: integer-size(32),
      #   _toc_info :: binary >> = vbri_header
